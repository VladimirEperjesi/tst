...
<xsl:param name="system" select="document('param:system')/*"/>  <!-- ancestor::system -->
...
<xsl:variable name="columns" select="$instanceTables/*/columns"/>
...
<xsl:template name="entity">
	...
	<xsl:if test="count(preceding-sibling::table[@name=current()/@name])=0">
	...
	<step id="map_{$tableName}_source_columns" className="com.ataccama.dqc.tasks.flow.AlterFormat" disabled="false" mode="NORMAL">
        <properties>
            ...
            <mrg:mergeChildren key="@name">
				 <xsl:for-each select="$instanceTables/*[@name=$tableName]/columns/column[(@origin='source' and @artificial='false' and @load='true') or (@name='source_id' and @originalPk='true')]">
                	<xsl:call-template name="column_af"/>
                </xsl:for-each>
                <!-- <xsl:for-each select="columns/column[@isSrc='false']"> 
                	<columnDef name="{replace(@name,'dic_','')}" type="{@type}"/>
            	</xsl:for-each>-->
                <xsl:if test="@mode='autonomous' or @mode='central'">
	...
	<step id="{@name}" className="com.ataccama.dqc.tasks.common.usersteps.io.OutputStep" disabled="false" mode="NORMAL">
        <properties>
            <requiredColumns>
                ...
                <xsl:for-each select="$instanceTables/*[@name=$tableName]/columns/column[@origin='source']">
    ...
    <!-- LOOKUP VALID -->
    <xsl:for-each select="$instanceTables/physicalTable[@name=$tableName]/columns/column[@refData!='']">	
		...
		<xsl:variable name="isSrc" select="$instanceModel/tables/table[@name=$tableName]/columns/column[@refData=$dic]/@isSrc"/>		
...
<!-- generate AF with origin mapping -->
		...
		<xsl:apply-templates select="$system/sourceMappings/mapping[@entity=$tableName]">
			<xsl:with-param name="position" select="position()-1"/>
...
<xsl:template match="mapping">
	...
	<step id="map_internal_columns_from_{$origin_table}_to_{$origin_entity}" className="com.ataccama.dqc.tasks.flow.AlterFormat" disabled="false" mode="NORMAL">
        <properties>
            ...
            <!-- compound key -->
                <xsl:for-each select="$instanceTables/*[@name=$origin_entity]/columns/column[@artificial='true']">
					<addedColumn name="{lower-case(@name)}" type="STRING">
