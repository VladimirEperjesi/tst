...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="detailTemplate">
		...
		<xsl:variable name="dest_table_name" select="$masterModels/masterModel[@name=$model_name]/relationships/relationship[@name=current()/@relationship]/@childTable" />
		<!--  select="$masterModels/masterModel[@name=$model_name]/*/*[@name=$table_name]/guiTab/views/detailView/groups/labeledRelatedList[@relationship=current()/@relationship]/@name"  -->
				...
				<relationRowLabel>
					<xsl:attribute name="dataFormat">
						...
						<!--  todo <span style="color:red;">${id}</span>
			      		...
			      		<!-- todo tvy: this will be useful when it is possible to show lable on 1:N connection 
			      		<xsl:text>${rev_</xsl:text><xsl:value-of select="@relationship"/><xsl:text>.</xsl:text><xsl:value-of select="$masterModels/masterModel[@name=$model_name]/*/*[@name=$table_name]/guiTab/views/detailView/groups/labeledRelatedList[@relationship=current()/@relationship]/hierarchyParams/@relationshipType"/><xsl:text>}</xsl:text>
