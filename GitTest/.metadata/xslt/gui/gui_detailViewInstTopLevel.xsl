		...
		<slotOptions>
			...
			<option name="contentDataLabel">
				<xsl:attribute name="value">
		      		<xsl:for-each select="$masterModels/masterModel[lower-case(@name)=lower-case($model_name)]/instanceTables/instanceTable[lower-case(@name)=lower-case($table_name)]/guiTab/views/breadcrumbView/columns/column">
						...
						<div classes="mda-limitWidthContentContainer">
							<div classes="mda-columnsPanel">
								...
								<div id="bottom" style="display:inline-block; width:100%;">
									...
									<!--  instances -->
									<xsl:apply-templates select="$masterModels/masterModel[lower-case(@name)=lower-case($model_name)]/instanceTables/instanceTable[lower-case(@name)=lower-case($table_name)]/guiTab/views/detailView/groups/listGrid[@templatePosition='bottom']"/>
								</div>
							</div>
						</div>
					</div>
